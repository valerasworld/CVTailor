...
//        let italicFont = regularFont.withSymbolicTraits(.traitItalic)!
    ...
    func findVerticalSpacing(_ newPDFData: [(TextKind, String)], index: Int) -> CGFloat {
        
        guard index != newPDFData.count - 1 else {
            return 0
        }
        let currentTextKind = newPDFData[index].0
        let nextTextKind = newPDFData[index + 1].0
        ...
        if nextTextKind == .titleUnderlined {
            return 10
        }
        return 0
    }
    ...
    for index in 0..<newPDFData.count {
        let tuple = newPDFData[index]
        ...
        let verticalSpacing: CGFloat = findVerticalSpacing(newPDFData, index: index) // Отступ между текстовыми блоками
...
struct ContentViewViewView: View {
    var body: some View {
        VStack {
...
struct MyNewPDFData {
    let newPDFData: [(TextKind, String)] = [
